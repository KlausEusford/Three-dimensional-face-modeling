...
bool bSetupPixelFormat(HDC hdc)                //定义屏幕格式
{ 
...
void SizeOpenGLScreen(int width, int height)	//初始化模型视点矩阵
{
	if (height==0)										
	{
		height=1;										
...
void InitializeOpenGL(int width, int height)     //定义屏幕大小
{  
    g_hDC = GetDC(g_hWnd);							
